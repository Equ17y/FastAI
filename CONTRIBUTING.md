...
После активации виртуального окружения убедитесь, что всё настроено верно:

1. **Проверка активации:** В начале строки вашего терминала должен появиться префикс `(.venv)` или `(fastai)`.
2. **Проверка версии Python:** Выполните команду:
   ```shell
   $ python --version
...
### Подключение фронтенда для локального тестирования

Для ручной отладки бэкенда используется скомпилированный фронтенд. Он не хранится в репозитории, его нужно скачать локально.

1. Скачайте актуальный архив с фронтендом по ссылке: [Скачать фронтенд](https://dvmn.org/filer/canonical/1750917104/1034/)
2. Распакуйте архив в корень репозитория и переименуйте папку в `frontend`.
3. Убедитесь, что папка `frontend/` указана в файле `.gitignore`, чтобы не закоммитить её случайно.
...
В проекте используется [Ruff](https://docs.astral.sh/ruff/) для линтинга и форматирования.
...
**Форматирование кода (отступы, переносы строк):**

Просмотр, какие изменения предложит форматировщик, но не применять их:
```shell
$ ruff format --diff .
```

Применение форматирование ко всем файлам проекта:
```shell
$ ruff format .
```
